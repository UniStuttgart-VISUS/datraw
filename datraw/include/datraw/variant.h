...
namespace datraw {
    ...
    class DATRAW_API variant;


    /// <summary>
    /// Possible data types <see cref="datraw::variant" /> can hold.
    /// </summary>
    enum class DATRAW_API variant_type {
        /// <summary>
...
namespace detail {
    /// <summary>
    //// Actual storage structure for <see cref="datraw::variant" />.
    /// </summary>
    /// <remarks>
    /// <para>This union should never be used except for in
    /// <see cref="datraw::variant" />!</para>
    ...
    /// uninitialised memory. The <see cref="datraw::variant" /> must handle
    /// initialisation and finalisation on behalf of the union.</para>
    /// </remarks>
    union DATRAW_API variant {
        bool val_boolean;
    ...
    /// </remarks>
    template<variant_type T> struct variant_fwd_traits { };
    ...
    /// </remarks>
    template<class T> struct variant_rev_traits { };

#define __DATRAW_DECL_VARIANT_TYPE_TRAITS(t)                                   \
    template<> struct DATRAW_API variant_fwd_traits<variant_type::t> {         \
        typedef decltype(detail::variant::val_##t) type;                       \
    ...
    template<> struct DATRAW_API variant_rev_traits<                           \
            decltype(detail::variant::val_##t)> {                              \
        static const variant_type type = variant_type::t;                      \
        inline static decltype(detail::variant::val_##t) *get(                 \
                detail::variant& v) {                                          \
            return &v.val_##t;                                                 \
        }                                                                      \
        inline static const decltype(detail::variant::val_##t) *get(           \
    ...
    __DATRAW_DECL_VARIANT_TYPE_TRAITS(boolean);
    __DATRAW_DECL_VARIANT_TYPE_TRAITS(int8);
    __DATRAW_DECL_VARIANT_TYPE_TRAITS(int16);
    __DATRAW_DECL_VARIANT_TYPE_TRAITS(int32);
    __DATRAW_DECL_VARIANT_TYPE_TRAITS(int64);
    __DATRAW_DECL_VARIANT_TYPE_TRAITS(uint8);
    __DATRAW_DECL_VARIANT_TYPE_TRAITS(uint16);
    __DATRAW_DECL_VARIANT_TYPE_TRAITS(uint32);
    __DATRAW_DECL_VARIANT_TYPE_TRAITS(uint64);
    __DATRAW_DECL_VARIANT_TYPE_TRAITS(float32);
    __DATRAW_DECL_VARIANT_TYPE_TRAITS(float64);
    __DATRAW_DECL_VARIANT_TYPE_TRAITS(string);
    __DATRAW_DECL_VARIANT_TYPE_TRAITS(wstring);
    __DATRAW_DECL_VARIANT_TYPE_TRAITS(pointer);
    // Add new specialisations here here.

#undef __DATRAW_DECL_VARIANT_TYPE_TRAITS
    ...
    template<variant_type T> struct cast_to {
        typedef typename variant_fwd_traits<T>::type type;
    ...
    template<variant_type T> struct copy_to {
        typedef typename variant_fwd_traits<T>::type type;
        static inline void invoke(const type& v, datraw::variant& target) {
            target.set(v);
        }
    };
    ...
    template<variant_type T> struct is_same {
        typedef typename variant_fwd_traits<T>::type type;
        static inline void invoke(type& v, const datraw::variant& rhs,
                bool& retval) {
            retval = (v == rhs.get<T>());
        }
    };
    ...
    template<variant_type T> struct move_to {
        typedef typename variant_fwd_traits<T>::type type;
        static inline void invoke(type& v, datraw::variant& target) {
    ...
    public:
        ...
        const typename detail::variant_fwd_traits<T>::type& get(void) const {
            assert(this->cur_type == T);
            return *detail::variant_fwd_traits<T>::get(this->data);
        }
        ...
        template<class T> const T& get(void) const {
            ...
            return *detail::variant_rev_traits<T>::get(this->data);
        }
        ...
        template<variant_type T>
        void set(const typename detail::variant_fwd_traits<T>::type& value) {
            this->reconstruct<T>();
        ...
        template<class T> void set(const T& value) {
            ...
            typedef detail::variant_rev_traits<type> traits;
        ...
        /// <tparam name="T">The new type of the variant.</tparam>
        template<variant_type T>
        void set(typename detail::variant_fwd_traits<T>::type&& value) {
            this->reconstruct<T>();
        ...
        template<variant_type T> inline variant& operator =(
                const typename detail::variant_fwd_traits<T>::type& rhs) const {
            this->set(rhs);
            return *this;
        }
    ...
    private:
        ...
        template<variant_type T> inline void reconstruct(void) {
            typedef typename detail::variant_fwd_traits<T> traits;
