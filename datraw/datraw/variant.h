...
#if !defined(_DATRAW_VARIANT_H)
#define _DATRAW_VARIANT_H
#pragma once
...
enum class variant_type {
    ...
    // Add new members here. Do not forget to add them in the dispatch list
    // below, too.
};
...
/// destructor, which results in its members being treated like uninitialised
/// memory. The <see cref="datraw::basic_variant" /> must handle initialisation
/// and finalisation on behalf of the union. You will most likely get that wrong
/// if you use this type directly.</para>
/// </remarks>
union variant final {
    bool val_boolean;
    std::int8_t val_int8;
...
/// <summary>
/// Resolves the C++ type for the given <paramref name="Variant" /> type.
/// </summary>
/// <typeparam name="Variant">The variant to resolve the C++ representation of.
/// </typeparam>
template<variant_type Variant>
using variant_t = typename variant_fwd_traits<char, Variant>::value_type;


/// <summary>
/// Resolves the <see cref="variant_type" /> for the given C++ type.
/// </summary>
/// <typeparam name="TType">The type to resolve the variant discriminant for,
/// which must be one of the valid variant types that are described by traits of
/// the library.</typeparam>
template<class TType>
constexpr variant_type variant_type_v = variant_rev_traits<char, TType>::type;
...
/// </summary>
/// <tparam name="C">The character type used for strings.</tparam>
template<class C> class basic_variant final {
...
public:
    ...
    /// </summary>
    inline basic_variant(void) noexcept : cur_type(variant_type::empty) { }
    ...
    inline basic_variant(basic_variant&& rhs) noexcept
            : cur_type(variant_type::empty) {
        *this = std::move(rhs);
    }
    ...
    /// </summary>
    inline ~basic_variant(void) noexcept {
        this->clear();
    }
    ...
    /// </summary>
    /// <remarks>
    /// The method does not check (except for an assertion) whether it is legal
    /// to retrieve the value as the specified type. It is up to the user to
    ...
    inline bool is(const variant_type type) const noexcept {
    ...
    void set(const detail::variant_t<T>& value) {
    ...
    /// <tparam name="T">The new type of the variant.</tparam>
    template<variant_type T>
    void set(detail::variant_t<T>&& value) {
        typedef detail::variant_fwd_traits<char_type, T> traits;
        this->reconstruct<T>();
    ...
    inline variant_type type(void) const noexcept {
    ...
    template<variant_type T>
    inline basic_variant& operator =(const detail::variant_t<T>& rhs) const {
        this->set(rhs);
        return *this;
    }
    ...
    /// are equal.</returns>
    bool operator ==(const basic_variant& rhs) const noexcept;
    ...
    inline bool operator !=(const basic_variant& rhs) const noexcept {
        return !(*this == rhs);
    }
...
#endif /* !defined(_DATRAW_VARIANT_H) */
